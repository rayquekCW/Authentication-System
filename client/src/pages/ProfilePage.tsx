...
const ProfilePage = () => {
    ...
    if (getSession) {
      getSession()
        .then(async (sessionData: any) => {
          ...
          setUserData({
            sub: sessionData.sub,
            name: sessionData.given_name + " " + sessionData.family_name,
  ...
  return (
    <>
      ...
      <div
        className={`overlay ${
          showSignInPopUp || showDeleteConfirmPopup || showChangeConfirmPopup
            ? "active"
            : ""
        }`}
      ></div>
        ...
        <table className="table h-50 text-center">
          <tbody>
            ...
            <tr>
              ...
              <td className="text-start p-3">{userData?.email}</td>
            </tr>
            <tr>
              <th className="text-start p-3">Phone Number</th>
              <td className="text-start p-3">
                {userData?.phone_number ? userData.phone_number : "Not Set"}
              </td>
            </tr>
            <tr>
              <th className="text-start p-3">Birth Date</th>
              <td className="text-start p-3">{userData?.birthdate}</td>
            </tr>
          </tbody>
        </table>
        {isCognitoUser && (
      ...
      {isCognitoUser && mfaEnabled && isPhoneVerified && (
        <div className="container bg-light shadow-sm mt-4 p-4">
          ...
          <div className="row px-3">
            <div className="pb-3">
              <div>
                To secure your login identities, we will use OTP verification
              </div>
            ...
            <div className="my-2">
              <div className="form-check pb-2">
                <input
                  ...
                  id="sms-radio"
                  value="SMS_MFA"
                  onChange={(event) => setMfaPreference(event.target.value)}
              ...
              <div className="form-check">
                <input
                  ...
                  onChange={(event) => setMfaPreference(event.target.value)}
                  checked={mfaPreference === "SOFTWARE_TOKEN_MFA"}
                />
